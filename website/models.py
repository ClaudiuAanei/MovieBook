...
class User(db.Model, UserMixin):
    ...
    movie_rel: Mapped[list["RandomMovie"]] = relationship("RandomMovie", back_populates="user_rel")
...
class RandomMovie(db.Model):
    ...
    poster_url: Mapped[str] = mapped_column(String(10), nullable=False)
    movie_owner: Mapped[int] = mapped_column(Integer, ForeignKey('users.id'), nullable=False)

    user_rel: Mapped[list["User"]] = relationship("User", back_populates="movie_rel")
